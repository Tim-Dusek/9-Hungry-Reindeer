...
int main(int argc, char *argv[]) {
	...
	string threadPad = "_p4";
	
	printf("Program 1 has recieved file name: %s\n", inputFileName.c_str());
	string line = "";
	...
	if (semget(semID, 1, IPC_CREAT | 0666)==0){ //exit if error getting semaphore
		exit(1);
	}
	srand(time(NULL));
    while(fileCountNumber <= completeString.length()) {
		...
		randomNumber = rand() % 9 + 1; //random number 1-10
		threadPad.pop_back();
		threadPad = threadPad + std::to_string(randomNumber);
	...
	sem_operator.sem_flg = 0;
	semop(sem, &sem_operator, 1); //add 2 to semop
    inFile.close();
	return 0;
}
